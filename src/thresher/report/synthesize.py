...
def _generate_agent_report(
    ...
    """Invoke the synthesis agent to produce the final report.

    Delegates to ``thresher.agents.synthesize.run_synthesize_agent``.
    Falls back to template-based report generation on failure.
    """
    from thresher.agents.synthesize import run_synthesize_agent

    synthesis_input = _build_synthesis_input(scanner_results, ai_findings, enriched)

    agent_succeeded = run_synthesize_agent(config, report_dir, synthesis_input)
